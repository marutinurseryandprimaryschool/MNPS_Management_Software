/* ============================================
   ...
   ============================================
   exceljs workbook builders and jspdf equivalents for the Principal's
   balance sheet (daily + monthly) and the defaulter list
   (docs/designs/principal-role-fees-accounts.md, D17 / Issue 6A).
...
export async function exportDailyBalanceSheetExcel(data: DailySheetData): Promise<void> {
  ...
  ws.columns = [
    { width: 18 }, { width: 24 }, { width: 18 }, { width: 22 }, { width: 14 }, { width: 14 },
  ];

  let row = addSheetTitle(ws, data, `Daily Balance Sheet — ${data.dateKey}`);
  ...
  ws.getCell(row, 1).value = 'Fee Collections';
  ws.getCell(row, 1).font = SECTION_FONT;
  row = addSheetTable(ws, row + 1,
    ['Receipt', 'Student', 'Class', 'Category', 'Mode', 'Amount'],
    data.payments.map(p => [p.receipt, p.student, p.className, p.category, p.mode, p.amount]),
    [5]);
  row += 1;
  ...
  await downloadWorkbook(wb, `balance-sheet-${data.dateKey}.xlsx`);
}
...
export async function exportMonthlyBalanceSheetExcel(data: MonthlySheetData): Promise<void> {
  const wb = await newWorkbook();
  const ws = wb.addWorksheet('Monthly Sheet');
  ws.columns = [{ width: 14 }, { width: 14 }, { width: 14 }, { width: 14 }, { width: 14 }, { width: 14 }];

  let row = addSheetTitle(ws, data, `Monthly Balance Sheet — ${data.monthLabel}`);

  row = addSheetTable(ws, row,
    ['Date', 'Cash In', 'Bank In', 'Cash Out', 'Bank Out', 'Net'],
    [
      ...data.days.map(d => [
        d.dateKey, d.cashIn, d.bankIn, d.cashOut, d.bankOut,
        d.cashIn + d.bankIn - d.cashOut - d.bankOut,
      ] as (string | number)[]),
      ['Total',
        data.days.reduce((s, d) => s + d.cashIn, 0),
        data.days.reduce((s, d) => s + d.bankIn, 0),
        data.days.reduce((s, d) => s + d.cashOut, 0),
        data.days.reduce((s, d) => s + d.bankOut, 0),
  ...
  await downloadWorkbook(wb, `balance-sheet-${data.monthKey}.xlsx`);
}
...
export async function exportDailyBalanceSheetPdf(data: DailySheetData): Promise<void> {
  ...
  let y = pdfTitle(doc, data, `Daily Balance Sheet — ${data.dateKey}`);
  ...
  y = pdfSection(doc, 'Fee Collections', y);
  y = pdfTable(doc, [
    { header: 'Receipt', width: 34 },
    { header: 'Student', width: 44 },
    { header: 'Class', width: 20 },
    { header: 'Category', width: 40 },
    { header: 'Mode', width: 20 },
    { header: 'Amount', width: 26, align: 'right' },
  ], data.payments.map(p => [p.receipt, p.student, p.className, p.category, p.mode, inr(p.amount)]), y) + 4;
  ...
  doc.save(`balance-sheet-${data.dateKey}.pdf`);
}
...
export async function exportMonthlyBalanceSheetPdf(data: MonthlySheetData): Promise<void> {
  const doc = await newPdf('p');
  let y = pdfTitle(doc, data, `Monthly Balance Sheet — ${data.monthLabel}`);

  y = pdfTable(doc, [
    { header: 'Date', width: 30 },
    { header: 'Cash In', width: 30, align: 'right' },
    { header: 'Bank In', width: 30, align: 'right' },
    { header: 'Cash Out', width: 30, align: 'right' },
    { header: 'Bank Out', width: 30, align: 'right' },
    { header: 'Net', width: 30, align: 'right' },
  ], [
    ...data.days.map(d => [
      d.dateKey, inr(d.cashIn), inr(d.bankIn), inr(d.cashOut), inr(d.bankOut),
      inr(d.cashIn + d.bankIn - d.cashOut - d.bankOut),
    ]),
    ['Total',
      inr(data.days.reduce((s, d) => s + d.cashIn, 0)),
      inr(data.days.reduce((s, d) => s + d.bankIn, 0)),
      inr(data.days.reduce((s, d) => s + d.cashOut, 0)),
      inr(data.days.reduce((s, d) => s + d.bankOut, 0)),
      inr(data.totalIncome - data.totalExpense)],
  ], y) + 4;

  y = pdfSection(doc, 'Closing Balances (end of month)', y);
  pdfTable(doc, [{ header: '', width: 90 }, { header: 'Amount', width: 60, align: 'right' }], [
    ['Cash in Hand', inr(data.cashInHand)],
    ['Bank Balance', inr(data.bankBalance)],
    ['Total', inr(data.cashInHand + data.bankBalance)],
  ], y);

  doc.save(`balance-sheet-${data.monthKey}.pdf`);
}
